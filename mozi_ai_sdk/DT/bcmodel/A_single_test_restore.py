...
def multi_layer_cnn(rnn_type, matrix, batch):
    ...
    cnn_layer_1 = conv(input_1, 2, 16, 2, "cnn_1")
    cnn_layer_2 = conv(cnn_layer_1, 6, 32, 3, "cnn_2")
    cnn_layer_3 = conv(cnn_layer_2, 12, 256, 4, "cnn_3", padding="VALID")
    ...
    if rnn_type == "lstm":
        cell = tf.contrib.rnn.BasicLSTMCell(
            num_units=lstm_hidden_size, state_is_tuple=False
        )
    else:
    ...
    with tf.Session() as session:
        if os.path.exists("new_2_8_12_tmp_5/checkpoint"):
            saver.restore(session, "new_2_8_12_tmp_5/pcgrad_regression.ckpt")
