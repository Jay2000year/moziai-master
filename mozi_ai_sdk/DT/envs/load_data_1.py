...
parser.add_argument(
    "--path", type=str, default="C:\\Users\\3-5\\Desktop\\aircraft_missile\\"
)
...
# 矩阵大小设置
parser.add_argument("--matrix_size", type=int, default=400)
# 中点坐标
parser.add_argument(
...
class ConstructionMatrix:
    def __init__(self):
        ...
        # # 导入飞机数据
        # self.red_unit = pd.read_excel(self.args.path+self.args.total_unit_file_name)
        # # 数据处理
        # self.null_row = self.red_unit['Time'].isnull()
        # # 导入导弹数据
        # self.enemy_unit = pd.read_excel(self.args.path+self.args.total_enemy_file_name)
        # # 导入rtg数据
        # self.rtg = pd.read_excel(self.args.path + self.args.total_rtg_file_name)
        # # 删除红方单元数据读取中的空行
        # self.remove_null()
        # 生成矩阵范围
        self.generate_zone()
        ...
        for idx, (ind, row) in pbar:
            ...
            if idx in step_lst:
                ...
                if population_input:
                    ...
                    # 将返回的三维数据添加到列表中
                    three_dimensional_matrix = three_dimensional_matrix.transpose(
                        2, 0, 1
                    )
    ...
    def get_bounds_point(self, lat, lon):
        ...
        时间：7/22/22
        """
        dic = self.get_point_with_point_bearing_distance(lat, lon, 0, 216)
        dic_1 = self.get_point_with_point_bearing_distance(lat, lon, 90, 216)
    ...
    def generate_zone(self):
        ...
        len_1, len_2 = (
            abs(northeast[1] - northwest[1]) / self.args.matrix_size,
            abs(northeast[0] - northwest[0]) / self.args.matrix_size,
        )
        ...
        len_3, len_4 = (
            abs(northwest[1] - southwest[1]) / self.args.matrix_size,
            abs(northwest[0] - southwest[0]) / self.args.matrix_size,
        )
    ...
    def matrix_create(self):
        ...
        speed_pos_matrix = np.zeros(
            shape=(self.args.matrix_size, self.args.matrix_size), dtype=np.float64
        )
        # 高度二维数据
