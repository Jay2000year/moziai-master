...
from mozi_ai_sdk.DT.PCGrad_tf import PCGrad
...
def multi_layer_cnn(rnn_type):
    ...
    input_1 = tf.placeholder(tf.float32, [None, 1000, 1000, 4])
    ...
    # label = tf.Variable(tf.random_uniform((200, 3, 1), minval=-1, maxval=1, dtype=tf.float32))

    cnn_layer_1 = conv(input_1, 8, 16, 4, "cnn_1")
    cnn_layer_2 = conv(cnn_layer_1, 4, 32, 2, "cnn_2")
    cnn_layer_3 = conv(cnn_layer_2, 11, 256, 1, "cnn_3", padding="VALID")
    ...
    a1_head = fc(
        a11_head_in, 1, "a1_head", in_channels=lstm_hidden_size
    )  # output: [batch_size, hidden_size]
    a1_embedding = fc(a1_head, 256, "a1_embedding", in_channels=1)

    a2_head_in = tf.add(multi_head_in, a1_embedding)
    a2_head = fc(
    ...
    for epoch in range(5):
        for index in range(len(batch_list)):
            ...
            # x_data = state.eval()
            # y_data = action.eval()
            # x_data = session.run(state)
            # y_data = session.run(action)
    ...
    saver.save(session, "new64_8_4_9_tmp_10/pcgrad_regression.ckpt")
