...
import time
from MoziDemo.envs.common.utils import *
...
class Environment(BaseEnvironment):
    ...
    def __init__(self, IP, AIPort, platform, scenario_name, simulate_compression, duration_interval, synchronous):
        super(Environment, self).__init__(ip=IP,
                                          port=AIPort,
                                          platform=platform,
                                          scenario_name=scenario_name,
                                          simulate_compression=simulate_compression,
    ...
    def reset(self, side_name):
        ...
        self.redside = self.scenario.get_side_by_name('红方(防御)')
        self.redside.static_construct()
        self.blueside = self.scenario.get_side_by_name('蓝方(进攻)')
    ...
    #     """
    #
    #     def check_linux_mozi_server(container_id):
    #         linux_password = "123456\n"
    #         _sudo = sh.sudo.bake("-S", _in=linux_password)
    #         sh.cd('/home/hsfw/')
    #         pid = 0
    #         try:
    #             pid = _sudo.sh('check_mozi_status.sh', container_id)
    #             pid = pid.stdout.decode().split('\n')
    #             i = 0
    #             for p in pid:
    #                 if p != '': i += 1
    #             return i
    ...
    #     time.sleep(10)
    #
    #     with open("/home/hsfw/docker_manager.txt", "r") as f:
    #         for str in f.readlines():
    #             docker_info = str.split(',')
    #             if check_linux_mozi_server(docker_info[0]) == 0:
    #                 container_id = docker_info[0]
    #                 server_ip = docker_info[1]
    #                 aiPort = docker_info[-1].strip('\n')
    #                 break
    #     if container_id == None:
    #         raise Exception('No docker resources available!')
    #     self.mozi_server = MoziServer(server_ip, aiPort, self.platform, self.scenario_name,
    #                                   self.simulate_compression, self.synchronous, container_id)
    #     time.sleep(time_out)
