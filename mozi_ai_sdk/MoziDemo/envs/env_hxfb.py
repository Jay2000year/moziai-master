...
class HXFBEnv(MultiAgentEnv):
    ...
    def _defensive_air_mission_action(self, zone):
        def act(side, mission_unit):
            ...
            side.add_mission_patrol(mission_name, 0, zone)  # 空战巡逻
    ...
    def _offensive_air_mission_action(self, zone):
        def act(side, mission_unit):
            ...
            self.side.add_mission_patrol(mission_name, 0, zone)  # 空战巡逻
    ...
    def _attack_anti_surface_ship_mission_action(self, target):
        def act(side, mission_unit):
            ...
            side.add_mission_strike(mission_name, 2)
    ...
    def _create_or_update_battle_zone(self):
        ...
        if len(defaults) < 4:
            ...
            side.add_reference_point(self.side_name, zone[0], patrolBoundingBox[0]['latitude'],
                                     patrolBoundingBox[0]['longitude'])
            side.add_reference_point(self.side_name, zone[1], patrolBoundingBox[1]['latitude'],
                                     patrolBoundingBox[1]['longitude'])
            side.add_reference_point(self.side_name, zone[2], patrolBoundingBox[2]['latitude'],
                                     patrolBoundingBox[2]['longitude'])
            side.add_reference_point(self.side_name, zone[3], patrolBoundingBox[3]['latitude'],
    ...
    def _create_or_update_offensive_patrol_zone(self):
        ...
        if len(Offensive_rp) == 0:
            side.add_reference_point(self.side_name, zone[0], hostileContactBoundingBox[0]['latitude'],
                                     hostileContactBoundingBox[0]['longitude'])
            side.add_reference_point(self.side_name, zone[1], hostileContactBoundingBox[1]['latitude'],
                                     hostileContactBoundingBox[1]['longitude'])
            side.add_reference_point(self.side_name, zone[2], hostileContactBoundingBox[2]['latitude'],
                                     hostileContactBoundingBox[2]['longitude'])
            side.add_reference_point(self.side_name, zone[3], hostileContactBoundingBox[3]['latitude'],
    ...
    def _create_or_update_defensive_patrol_zone(self):
        ...
        if len(rps) != 5:
            # 巡逻任务1
            side.add_reference_point(self.side_name, 'rp2', rp12mid['latitude'], rp12mid['longitude'])
            side.add_reference_point(self.side_name, 'rp3', rp13mid['latitude'], rp13mid['longitude'])
            side.add_reference_point(self.side_name, 'rp4', rp14mid['latitude'], rp14mid['longitude'])
            side.add_reference_point(self.side_name, 'rp5', rp23mid['latitude'], rp23mid['longitude'])
            side.add_reference_point(self.side_name, 'rp6', rp34mid['latitude'], rp34mid['longitude'])
        elif len(rps) == 5:
            # 巡逻任务1
