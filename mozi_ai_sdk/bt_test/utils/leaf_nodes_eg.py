...
def create_antisurfaceship_mission(side_name, scenario):
    ...
    # 闪电#3 #4
    airs_2 = {
        k: v for k, v in airs_dic.items() if 3 <= int(re.sub("\D", "", v.strName)) <= 4
    }
    ...
    wayPointList1 = [
        {"latitude": "26.0979297169117", "longitude": "153.365146994643"},
    ...
    wayPointList2 = [
        {"latitude": "25.2543871879078", "longitude": "153.238096612711"},
...
def update_antisurfaceship_mission(side_name, scenario):
    ...
    airs_dic = side.aircrafts
    airsOnMssn = {
        k: v for k, v in airs_dic.items() if v.strActiveUnitStatus.find("正在执行任务") > 0
    }
...
def update_patrol_mission(side_name, scenario):
    ...
    for patrol_mission in patrol_missions:
        ...
        if patrol_mission.strName == side_name + "xl3":
            ...
            list_str = [
                air.m_AssignedMission
                for air in air_other
            ...
            air_sky = [
                air
                for air in air_other
                if "按计划" in air.strActiveUnitStatus or "参与" in air.strActiveUnitStatus
            ]
...
def create_patrol_mission(side_name, scenario):
    ...
    if flag == "Yes":
        return False
    airs_c = [
        v for v in airs_dic.values() if 4 < int(re.sub("\D", "", v.strName)) < 9
    ]  # 编号5，6,7,8的,4架飞机
    for air in airs_c:
