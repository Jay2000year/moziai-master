...
def check_unit_retreat_and_compute_retreat_pos(side, unit_guid):
    ...
    for k, v in AirContacts.items():
        if v.m_IdentificationStatus >= 3:
            disKilo = g_get_two_point_distance(
            ...
            if disKilo <= v.fAirRangeMax * 1.852:  # 海里转公里需要乘以1.852
                ...
                strkmssn = [
                    v for v in mssnSitu.values() if v.strGuid == unit.m_AssignedMission
                ][0]
                print(unit.strName, "  ", strkmssn.strName)
                retreatPos = {
                    "latitude": 25.5307909534701,
                    "longitude": 153.239828460436,
                }
...
def assign_planway_to_mission(
    side, mission_name, way_name, way_point_list, old_way_name=None
):
    """
        给任务分配(重新分配)预设航线
    :param side: 方
...
def FindBoundingBoxForGivenContacts(side, padding=10):
    ...
    coordinates = [
        btBas.MakeLatLong(defaults[0]["latitude"], defaults[0]["longitude"]),
