...
class Model(object):
    def __init__(
        ...
        *,
        ...
        act_model = policy(
            sess, scope_name, ob_space, ac_space, nbatch_act, 1, reuse=False
        )
        train_model = policy(
            sess,
            scope_name,
        ...
        if value_clip:
            ...
            vf_loss = 0.5 * tf.reduce_mean(vf_losses1)
        ...
        def train(
            lr, cliprange, obs, returns, dones, actions, values, neglogpacs, states=None
        ):
            advs = returns - values
            ...
            return sess.run(
                [pg_loss, vf_loss, entropy, approxkl, clipfrac, summary_op, _train],
                td_map,
            )[:-1]
        ...
        self.writer = tf.summary.FileWriter(
            "./bin/tfevents_ppo/", graph=tf.get_default_graph()
        )
...
class PPOActor(object):
    def __init__(
        ...
        if enable_push:
            ...
            self._push_thread = Thread(
                target=self._push_data,
                args=(self._zmq_context, learner_ip, port_B, self._data_queue),
            )
    ...
    def run(self):
        ...
        while True:
            ...
            t = time.time()
            self._update_model()
            tprint("Update bcmodel time: %f" % (time.time() - t))
            t = time.time()
            # rollout
            num += 1
            ...
            if self._enable_push:
                if self._data_queue.full():
                    tprint("[WARN]: Actor's queue is full.")
    ...
    def _nstep_rollout(self, num):
        ...
        if isinstance(self._obs, tuple):
            mb_obs = tuple(
                np.asarray(obs, dtype=self._obs[0].dtype) for obs in zip(*mb_obs)
            )
        else:
        ...
        for t in reversed(range(self._unroll_length)):
            ...
            delta = (
                mb_rewards[t]
                + self._gamma * next_values * next_nonterminal
                - mb_values[t]
            )
            mb_advs[t] = last_gae_lam = (
                delta + self._gamma * self._lam * next_nonterminal * last_gae_lam
            )
    ...
    def _update_model(self):
        self._model_requestor.send_string("request bcmodel")
...
class PPOLearner(object):
    def __init__(
        ...
        if init_model_path is not None:
            self._model.load(init_model_path)
    ...
    def run(self):
        ...
        while True:
            while (
                self._learn_act_speed_ratio > 0
                and updates * self._batch_size
            ...
            if updates % self._print_interval == 0:
                ...
                tprint(
                    "Update: %d	Train-fps: %.1f	Rollout-fps: %.1f	"
    ...
    def _pull_data(self, zmq_context, data_queue, episode_infos, unroll_split, port_B):
                                ...
                                [
                                    list(
                                        zip(
                                            *transform_tuple(
                                                data[0],
                                                lambda x: np.split(x, unroll_split),
                                            )
                                        )
                                    )
                                ]
...
class PPOSelfplayActor(object):
    def __init__(
        ...
        init_opponent_pool_filelist=None,
    ...
    def run(self):
        while True:
            t = time.time()
            self._update_model()
            tprint("Time update bcmodel: %f" % (time.time() - t))
            t = time.time()
    ...
    def _update_model(self):
        ...
        if (
            not self._freeze_opponent_pool
            and random.uniform(0, 1.0) < self._model_cache_prob
        ):
    ...
    def _update_opponent(self):
        if (
            random.uniform(0, 1.0) < self._prob_latest_opponent
            or len(self._model_cache) == 0
        ):
            self._oppo_model.load_params(self._latest_model)
            tprint("Opponent updated with the current bcmodel.")
        else:
            model_params = random.choice(self._model_cache)
            self._oppo_model.load_params(model_params)
            tprint(
                "Opponent updated with the previous bcmodel. %d models cached."
