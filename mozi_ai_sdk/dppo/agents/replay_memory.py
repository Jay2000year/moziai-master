...
class RemoteReplayMemory(object):
    def __init__(
        ...
        if is_server:
            ...
            self._receiver_threads = [
                Thread(
                    target=self._server_proxy_worker,
                    args=(
                        self._zmq_context,
                        ports,
                    ),
            ...
            self._receiver_threads += [
                Thread(
                    target=self._server_receiver_worker,
                    args=(
                        self._zmq_context,
                        ports[1],
                    ),
    ...
    def push(self, *args):
        ...
        if (
            self._memory.total >= self._memory_warmup_size
            and self._memory.total >= self._block_size
            and self._memory.total % self._send_interval == 0
        ):
    ...
    def sample(self, batch_size, reuse_ratio=1.0):
        ...
        while (
            self._num_used / reuse_ratio >= self._num_received
            or self._memory_warmup_size > len(self._cache_blocks) * self._block_size
        ):
            time.sleep(0.001)
        batch = [
            random.choice(random.choice(self._cache_blocks)) for _ in range(batch_size)
        ]
...
if __name__ == "__main__":
    ...
    if job_name == "client":
        replay_memory = RemoteReplayMemory(
            is_server=False, memory_size=10000, memory_warmup_size=16
        )
        while True:
        ...
        replay_memory = RemoteReplayMemory(
            is_server=True, memory_size=10000, memory_warmup_size=10000
        )
        while True:
