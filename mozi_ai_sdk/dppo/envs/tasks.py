...
class Task(object):
    def __init__(self, env, scenario, sideName):
        ...
        self.time_zone_combine = list(
            itertools.product(
                [x for x in range(self.times)],
                [y for y in range(len(self.defend_zones))],
            )
        )
        ...
        self._actions = list(
            chain.from_iterable(
                [
                    self._Action(
                        "donothing", self._ActionDoNothing, self._DoNothingIsValid
                    ),
                    self._Action(
                        "defensive",
                        self._DefensiveAirMissionAction,
                        self._PatrolMissionIsValid,
                    ),
                    self._Action(
                        "offensive",
                        self._OffensiveAirMissionAction,
                        self._PatrolMissionIsValid,
                    ),
                    self._Action(
                        "attack",
                        self._AttackAntiSurfaceShipMissionAction,
    ...
    def _Action(self, type, function, is_valid):
        if type == "donothing":
            func_list = []
            func_list.append(
                Function(type=type, function=function(), is_valid=is_valid())
            )
            return func_list
        elif type == "defensive":
            func_list = []
            for times, i in self.time_zone_combine:
                ...
                func_list.append(
                    Function(
                        type=type,
                        function=function(missionName, times, zone),
                        is_valid=is_valid(),
                    )
                )
            return func_list
            ...
            for times in range(self.times):
                ...
                func_list.append(
                    Function(
                        type=type,
                        function=function(missionName, times, self.offend_zone),
                        is_valid=is_valid(),
                    )
                )
            return func_list
            ...
            for times in range(5, self.times * 2):
                ...
                func_list.append(
                    Function(
                        type=type,
                        function=function(missionName, times, self.target),
                        is_valid=is_valid(),
                    )
                )
            return func_list
    ...
    def _DefensiveAirMissionAction(self, missionName, times, zone):
        def act(missionUnit):
            ...
            patrolmssn = [
                v for _, v in side.patrolmssns.items() if v.strName == missionName
            ]
            if len(patrolmssn) != 0:
                return False
            scen_time = "04/16/2020 22:00:00"
            mission_time = datetime.datetime.strptime(
                scen_time, "%m/%d/%Y %H:%M:%S"
            ) + datetime.timedelta(minutes=self.delta * times)
            ...
            doctrineParam = {
                "emc_radar": "Passive",
                "evadeAuto": "true",
                "ignorePlottedCourse": "yes",
                "targetsEngaging": "true",
                "ignoreEmcon": "false",
                "weaponControlAir": "0",
                "weaponControlSurface": "0",
                "fuelStateForAircraft": "0",
                "fuelStateForAirGroup": "3",
                "weaponStateForAircraft": "2001",
    ...
    def _AttackAntiSurfaceShipMissionAction(self, missionName, times, target):
        def act(missionUnit):
            side = self.side
            strikemssn = [
                v for _, v in side.strikemssns.items() if v.strName == missionName
            ]
            if len(strikemssn) != 0:
                return False
            _target = target
