...
def check_unit_retreat_and_compute_retreat_pos(side, unit_guid):
    ...
    for k, v in AirContacts.items():
        if v.m_IdentificationStatus >= 3:
            disKilo = get_two_point_distance(
                unitPos["Longitude"], unitPos["Latitude"], v.dLongitude, v.dLatitude
            )
            if disKilo <= v.fAirRangeMax * 1.852:  # 海里转公里需要乘以1.852
                ...
                retreatPos = get_point_with_point_bearing_distance(
                    unitPos["Longitude"],
                    unitPos["Latitude"],
                    unit.fCurrentHeading + 180,
                    10,
                )
...
def FindBoundingBoxForGivenContacts(contacts, defaults, padding):
    ...
    coordinates = [
        btBas.MakeLatLong(
            defaults["AI-AO-1"]["latitude"], defaults["AI-AO-1"]["longitude"]
        ),
        btBas.MakeLatLong(
            defaults["AI-AO-2"]["latitude"], defaults["AI-AO-2"]["longitude"]
        ),
        btBas.MakeLatLong(
            defaults["AI-AO-3"]["latitude"], defaults["AI-AO-3"]["longitude"]
        ),
        btBas.MakeLatLong(
            defaults["AI-AO-4"]["latitude"], defaults["AI-AO-4"]["longitude"]
        ),
    ]
    ...
    for k, v in contacts.items():
        ...
        contactCoordinates.append(
            btBas.MakeLatLong(contact["latitude"], contact["longitude"])
        )

    # Get Hostile Contact Bounding Box
    if len(contactCoordinates) > 0:
        contactBoundingBox = FindBoundingBoxForGivenLocations(
            contactCoordinates, padding
        )
...
def FindBoundingBoxForGivenLocations(coordinates, padding):
    ...
    # Return In Format
    return [
        MakeLatLong(north, west),
        MakeLatLong(north, east),
        MakeLatLong(south, east),
        MakeLatLong(south, west),
    ]
...
def zone_contain_unit(zone, unit):
    ...
    zone = temp_1 + temp_2
    if (unit["latitude"] <= zone[0]["latitude"]) and (
        unit["latitude"] >= zone[2]["latitude"]
    ):
        if (unit["longitude"] <= zone[1]["longitude"]) and (
            unit["longitude"] >= zone[3]["longitude"]
        ):
            return True
        else:
            return False
...
def MakeLatLong(latitude, longitude):
    instance = {
        "latitude": InternationalDecimalConverter(latitude),
