...
class HXFBEnv(MultiAgentEnv):
    def __init__(self, env_config):
        ...
        self.asuw = {
            ...
            and (len(v.m_MultipleMissionGUIDs) == 0)
        }  # 可用反舰空战飞机
        self.s_asuw = dict(
            sorted(self.asuw.items(), key=lambda value: value[1].dLongitude)
        )
        self.asup = {
            k: v
            for k, v in self.side.aircrafts.items()
        ...
        self.target = {
            ...
            if v.m_ContactType == 2 and "DDG" in v.strName
        }
        self.s_asup = dict(
            sorted(self.asup.items(), key=lambda value: value[1].dLongitude)
        )

        self.action_atom_list = list(
            chain.from_iterable(
                [
                    [0 for _ in range(24)],
                    list(
                        itertools.product(
                            [x for x in self.s_asup.keys()],
                            [y for y in self.defend_zones],
                        )
                    ),
                    self.s_asuw.keys(),
                ]
        ...
        self._action_func_list = list(
            chain.from_iterable(
                [
                    ...
                    self._action(
                        "attack", self._attack_anti_surface_ship_mission_action
                    ),
                ]
            )
        )
    ...
    def step(self, action):
        ...
        if self.env_config["mode"] in ["train", "development"]:
            ...
            if mission_unit:
                self._action_func_list[action["agent_0"]].function(
                    self.side, mission_unit
                )
        ...
        return (
            obs,
            reward,
            {"__all__": done, "agent_0": done},
            {"agent_0": {"score": self.side.iTotalScore}},
        )
    ...
    def _generate_features(self):
        ...
        for k, v in self.side.aircrafts.items():
            ...
            temp_red_air_feats[5] = (
                self._get_weapon_num(weapon_list, [564, 2051, 3386]) / 10.0
            )
            # 空空导弹  51-AIM120D;945-AIM9X
            temp_red_air_feats[6] = self._get_weapon_num(weapon_list, [51, 945]) / 10.0
            # 反舰导弹  826-AGM154C
            temp_red_air_feats[7] = (
                self._get_weapon_num(
                    weapon_list,
                    [
                        826,
                    ],
        ...
        for k, v in self.side.ships.items():
            ...
            red_ship_feats = map(
                lambda x, y: x + y, red_ship_feats, temp_red_ship_feats
            )
        ...
        red_patrol_mission_feats = [
            self.side.patrolmssns.__len__() / 10.0,
        ]
        feats.extend(red_patrol_mission_feats)
        red_strike_mission_feats = [
            self.side.strikemssns.__len__() / 10.0,
        ]
    ...
    def _action(self, action_type, function):
        if action_type == "do-nothing":
            ...
            for atom in self.action_atom_list[24:-8]:
                ...
                patrol_zone = atom[1]
                func_list.append(
                    Function(type=action_type, function=function(patrol_zone))
                )
            return func_list
        ...
        if self._action_func_list[action].type == "do-nothing":
            ...
            if action_unit_key in self.asup.keys():  # 当前任务的战机是否存活
                ...
                if action_unit_class.m_AssignedMission == "":
                    ...
                    self.busy_asuw = {
                        k: v
                        for k, v in self.asuw.items()
                        if (
                            v.strAirOpsConditionString in [0, 11, 19, 20, 21]
                ...
                if action_unit_class.strAirOpsConditionString not in [
                    11,
                    19,
                    20,
                    21,
                ...
                if unit_value.m_AssignedMission == "":
                    ...
                    for mission_key, mission_value in side.patrolmssns.items():
                        if mission_key == unit_value.m_AssignedMission:
                            ...
                            time.sleep(1)
                            # 删除旧任务
                            lua = 'ScenEdit_DeleteMission("%s", "%s")' % (
                                self.side_name,
                                mission_value.strName,
                            )
    ...
    def _offensive_air_mission_action(self, zone):
        def act(side, mission_unit):
            ...
            OffensiveAirMiss = self.side.add_mission_patrol(
                mission_name, 0, zone
            )  # 空战巡逻
    ...
    def _create_or_update_battle_zone(self):
        ...
        defaults = {
            v.strName: {"latitude": v.dLatitude, "longitude": v.dLongitude}
            for k, v in side.referencepnts.items()
        ...
        if len(defaults) < 4:
            ...
            side.add_reference_point(
                zone[0],
                patrolBoundingBox[0]["latitude"],
                patrolBoundingBox[0]["longitude"],
            )
            side.add_reference_point(
                zone[1],
                patrolBoundingBox[1]["latitude"],
                patrolBoundingBox[1]["longitude"],
            )
            side.add_reference_point(
                zone[2],
                patrolBoundingBox[2]["latitude"],
                patrolBoundingBox[2]["longitude"],
            )
            side.add_reference_point(
                zone[3],
                patrolBoundingBox[3]["latitude"],
    ...
    def _create_or_update_offensive_patrol_zone(self):
        ...
        airContacts_dic = {
            k: v for k, v in side.contacts.items() if v.m_ContactType == 0
        }  # 探测到的敌方飞机
        if len(defaults) != 4:
            return

        hostileContactBoundingBox = FindBoundingBoxForGivenContacts(
            airContacts_dic, defaults, 1
        )

        if len(Offensive_rp) == 0:
            side.add_reference_point(
                zone[0],
                hostileContactBoundingBox[0]["latitude"],
                hostileContactBoundingBox[0]["longitude"],
            )
            side.add_reference_point(
                zone[1],
                hostileContactBoundingBox[1]["latitude"],
                hostileContactBoundingBox[1]["longitude"],
            )
            side.add_reference_point(
                zone[2],
                hostileContactBoundingBox[2]["latitude"],
                hostileContactBoundingBox[2]["longitude"],
            )
            side.add_reference_point(
                zone[3],
                hostileContactBoundingBox[3]["latitude"],
    ...
    def _create_or_update_defensive_patrol_zone(self):
        ...
        # 生成大四边形的四个中点
        rp12mid = MidPointCoordinate(
            aoPoints[0]["latitude"],
            aoPoints[0]["longitude"],
            aoPoints[1]["latitude"],
            aoPoints[1]["longitude"],
        )
        rp13mid = MidPointCoordinate(
            aoPoints[0]["latitude"],
            aoPoints[0]["longitude"],
            aoPoints[2]["latitude"],
            aoPoints[2]["longitude"],
        )
        rp14mid = MidPointCoordinate(
            aoPoints[0]["latitude"],
            aoPoints[0]["longitude"],
            aoPoints[3]["latitude"],
            aoPoints[3]["longitude"],
        )
        rp23mid = MidPointCoordinate(
            aoPoints[1]["latitude"],
            aoPoints[1]["longitude"],
            aoPoints[2]["latitude"],
            aoPoints[2]["longitude"],
        )
        rp34mid = MidPointCoordinate(
            aoPoints[2]["latitude"],
            aoPoints[2]["longitude"],
            aoPoints[3]["latitude"],
            aoPoints[3]["longitude"],
