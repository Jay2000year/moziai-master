...
def remote_connect(hostname, password):
    ...
    for i in range(MAX_DOCKER_RETRIES):
        try:
            client.connect(
                hostname=hostname, port=22, username="root", password=password
            )
            return client
...
def generate_docker(
    sever_docker_dict, password=None, path=None, remote_docker_port=None
):
    """
    ...
    for ip, docker_num in sever_docker_dict.items():
        ...
        if (
            ...
            generate_local_docker(
                ip, client, docker_num, sever_docker_info, remote_docker_port
            )
        else:
    ...
    try:
        for _ in range(MAX_DOCKER_RETRIES):
            ...
            out2 = container.exec_run(
                cmd='sh -c "pgrep mono"', tty=True, user="root", detach=False
            )
            if out2.output != b"":
...
def release_docker(docker_ip_port, password=None, path=None):
    ...
    if (
        docker_ip == "127.0.0.1"
        or docker_ip == "localhost"
        or docker_ip == get_local_ip()
        or docker_ip == get_public_ip()
    ):
        ...
        try:
            ...
            print(
                f"fail get or stop or remove mozi_{docker_ip}_{docker_port} container!"
            )
    else:
        assert password is not None
        assert path is not None
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sever_docker_ip_port", type=str, default="8.140.121.210:12354"
    )
    ...
    if args.mode == "create":
        sever_docker_dict = {
            args.sever_docker_ip: args.mono_num,
        }  # {'8.140.121.210': 3, '123.57.137.210': 2}
        if args.remote_docker_port:
            sever_docker_info = generate_docker(
                sever_docker_dict,
            ...
            sever_docker_info = generate_docker(
                sever_docker_dict, password=args.password, path=args.path
            )
        print("sever_docker_info: ", sever_docker_info)
    elif args.mode == "delete":
        sever_docker_dict = {
            args.sever_docker_ip: 0,
        }
        stop_docker(sever_docker_dict, password=args.password, path=args.path)
    elif args.mode == "restart":
        restart_mozi_container(
            args.sever_docker_ip_port, password=args.password, path=args.path
        )
