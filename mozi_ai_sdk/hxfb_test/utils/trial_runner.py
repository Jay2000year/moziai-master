...
def release_port(_training_id):
    try:
        ray_release_port.connect(
            "tcp://%s:%s" % (str(BACKEND_SERVER_IP), str(BACKEND_SERVER_PORT))
        )
        message = {"zmq_command": "train_complete", "training_id": _training_id}
...
def request_port(_schedule_addr, _schedule_port, _training_id, _num_workers):
    try:
        port_requestor.connect(
            "tcp://%s:%s" % (str(_schedule_addr), str(_schedule_port))
        )
        message = {}
...
class TrialRunner:
    ...
    def __init__(
        ...
        self._session_str = datetime.datetime.fromtimestamp(self._start_time).strftime(
            "%Y-%m-%d_%H-%M-%S"
        )
    ...
    def _execute_action(self, trial, decision):
        ...
        if decision == TrialScheduler.CONTINUE:
            ...
            # kill_proc()
            if (
                "env_config" in trial.config.keys()
                and "mode" in trial.config["env_config"].keys()
            ):
                if trial.config["env_config"]["mode"] == "train":
                    ...
                    # TODO
                    for rel_ip_port in trial.config["env_config"][
                        "avail_docker_ip_port"
                    ]:
