...
class WorkerSet:
        ...
        *,
        ...
        if _setup:
            ...
            self._local_worker = self._make_worker(
                RolloutWorker, env_creator, self._policy_class, 0, self._local_config
            )
    ...
    def add_workers(self, num_workers: int) -> None:
        ...
        remote_args = {
            ...
            "object_store_memory": self._remote_config["object_store_memory_per_worker"]
            or None,
    ...
    def _make_worker(
        ...
        # by dixit 2021/3/25
        if (
            "env_config" in self._remote_config.keys()
            and "env_config" in self._remote_config.keys()
            and "mode" in self._local_config["env_config"].keys()
            and "mode" in self._local_config["env_config"].keys()
        ):
            if self._remote_config["env_config"]["mode"] in [
                "train",
                "versus",
                "eval",
                "development",
            ] and self._local_config["env_config"]["mode"] in [
                "train",
                "versus",
                "eval",
                "development",
                ...
                if (
                    self._remote_config["env_config"]["avail_docker_ip_port"]
