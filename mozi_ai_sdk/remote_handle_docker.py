...
def restart_mozi_container(docker_ip_port, password='Hsfw123456', path='/root/anaconda3/envs/mozi38/lib/python3.8/site-packages/ray'):
    docker_ip = docker_ip_port.split(":")[0]
    get_local_ip()
    if docker_ip == '127.0.0.1' or docker_ip == 'localhost' or docker_ip == get_local_ip() or docker_ip == get_public_ip():
        restart_local_docker(docker_ip_port)
        os.system("docker cp /root/Mozi/bin/MoZi.ini mozi_127.0.0.1_6360:/home/LinuxServer/bin/")
        os.system("docker cp /root/Mozi/bin/DataOutput.ini mozi_127.0.0.1_6360:/home/LinuxServer/bin/")
    else:
        assert password is not None
        assert path is not None
...
def restart_local_docker(docker_ip_port):
    ...
    try:
        for _ in range(MAX_DOCKER_RETRIES):
            for _ in range(MAX_DOCKER_RETRIES):
                try:
                    container = client.containers.create('mozi_test_v22',
                                                         command='/bin/bash',
                                                         ...
                                                         ports={'6060': docker_port,
                                                                '2335': '2345',
                                                                '8778': '5506'},
                                                         # volumes=['/root/MoziLog/Analysis:/home/LinuxServer/bin/Analysis',
                                                         #          '/root/anaconda3/lib/python3.6/site-packages/mozi_ai_sdk/xun_code/scenarios:/home/LinuxServer/bin/Scenarios',],
                                                         volumes=['sys/fs/cgroup:sys/fs/cgroup',
                                                                  '/root/MoziServer/MapData:/root/working/MoziServer/MapData'],
                                                         user='root')
                except Exception:
                    print('container create fail')
                    print('container start over')
                try:
                    container.start()
                except:
            ...
            print("-----2----")
            container.exec_run(cmd='sh -c "cd /root/working/MoziServer/bin64/ && ./MZServer"',
                               tty=True,
                               user='root',
                               detach=True)
            time.sleep(2)
            out2 = container.exec_run(cmd='sh -c "ps -ef | grep MZServer | grep -v "grep" -c"',
                                      tty=True,
                                      user='root',
                                      detach=False)
            print('----1---')
