...
from mozi_ai_sdk.sc_code.envs.utils.geo import (
    plot_square,
    get_two_point_distance,
    get_cell,
    get_sudoku,
    convert_coordinate_to_lat_lon,
    convert_lua_obj_to_dict,
)
...
B_1_6 = (0, 0)
EXTERNAL_BOUNDARY = {
    "boundary_1": [(i, B_1_2[1]) for i in range(B_1_2[0] + 1)],  # (0->14, 0)
    "boundary_2": [
        (B_1_2[0], i) for i in range(B_1_2[1] + 1, B_2_3[1] + 1)
    ],  # (14, 1->8)
    "boundary_1_2": [B_1_2],  # 属于边界1
    "boundary_3": [
        (i, B_2_3[1]) for i in range(B_2_3[0] + 1, B_3_4[0] + 1)
    ],  # (15->29, 8)
    "boundary_2_3": [
        (B_2_3[0], B_2_3[1]),
        (B_2_3[0], B_2_3[1] + 1),
    ...
    "boundary_4": [
        (B_3_4[0], i) for i in range(B_3_4[1] + 1, B_4_5[1] + 1)
    ],  # (29, 9->29)
...
ANCHOR_POINTS = [
    (CARRIER_LOC[0] - 12, CARRIER_LOC[1] - 12),
...
ATTACK_POINTS = sum(
    map(
        lambda x: list(zip(range(x[0] - 3, x[0] + 4), range(x[1] + 3, x[1] - 4, -1))),
        ANCHOR_POINTS,
    ),
    [],
)
...
RULE_OPERATION_UNITS_TASK_STATUS = {
    "F-16A #21": {
        "first_stage": {"point": SCOUT_POINT_LIST[0], "status": False},
    ...
    "F-16A #23": {
        "first_stage": {"point": SCOUT_POINT_LIST[2], "status": False},
...
LOW_LEVEL_AGENT_NUM = (
    len(AIR_TO_AIR_GROUPS) + len(AIR_TO_SURFACE_GROUPS) + len(ELECTRONIC_JAMMER_GROUPS)
)
...
assert (
    len(AIR_TO_AIR_GROUPS)
    == len(AIR_TO_SURFACE_GROUPS)
    == len(ELECTRONIC_JAMMER_GROUPS)
    == len(MID_LEVEL_AGENT_GROUPS)
)
MID_LEVEL_AGENT_GROUPS_TO_OPERATION_GROUPS = {
    MID_LEVEL_AGENT_GROUPS[i]: [
        AIR_TO_AIR_GROUPS[i],
        AIR_TO_SURFACE_GROUPS[i],
...
def ml_agent_groups_to_ml_agent_id():
    ...
    for agent_group, op_groups in MID_LEVEL_AGENT_GROUPS_TO_OPERATION_GROUPS.items():
        ml_agent_groups_to_agent_id[agent_group] = [
            ML_AGENTS_TO_LL_AGENTS.inverse[OPERATION_GROUPS_TO_LL_AGENTS[op_group]]
...
HL_AGENT_ID_TO_ML_AGENT_GROUPS = {
    HIGH_LEVEL_AGENT_IDs[i]: MID_LEVEL_AGENT_GROUPS[i]
    for i in range(len(MID_LEVEL_AGENT_GROUPS))
}
HL_AGENTS_TO_ML_AGENTS = {
    HIGH_LEVEL_AGENT_IDs[i]: ml_agent_groups_to_ml_agent_id()[MID_LEVEL_AGENT_GROUPS[i]]
    for i in range(len(MID_LEVEL_AGENT_GROUPS))
}
...
LL_DATA_COLLECTOR = {agent_id: AgentCollector() for agent_id in LOW_LEVEL_AGENT_IDs}
HL_ML_AGENT_IDs = MID_LEVEL_AGENT_IDs + HIGH_LEVEL_AGENT_IDs
LAST_HL_ML_AGENTS_ACTION = {agent_id: None for agent_id in HL_ML_AGENT_IDs}
LAST_ML_AGENTS_OBS_REWARD_DONE = {agent_id: {} for agent_id in MID_LEVEL_AGENT_IDs}
HL_DATA_COLLECTOR = {agent_id: AgentCollector() for agent_id in HIGH_LEVEL_AGENT_IDs}
...
def restart_container(schedule_addr, schedule_port, _training_id, docker_ip_port):
    ...
    try:
        ...
        restart_requestor.connect(
            "tcp://%s:%s" % (str(schedule_addr), str(schedule_port))
        )
...
class MoziEnv(object):
    def __init__(self, env_config):
        ...
        self.env_config["avail_docker_ip_port"] = [
            "127.0.0.1:6060",
        ]
        self.reset_nums = 0
        self._get_env()
        ...
        self.mid_level_action_embed = {
            agent_id: local_seed.rand(MID_LEVEL_ACT_SPACE, MID_LEVEL_ACTION_EMBED)
            for agent_id in MID_LEVEL_AGENT_IDs
        }
        self.high_level_action_embed = {
            agent_id: local_seed.rand(HIGH_LEVEL_ACT_SPACE, HIGH_LEVEL_ACTION_EMBED)
    ...
    def _init_data_structure(self):
        ...
        self.air_to_air_groups = {
            AIR_TO_AIR_GROUPS[i]: {} for i in range(len(AIR_TO_AIR_GROUPS))
        }
        self.air_to_air_group_name_to_guid = {
            AIR_TO_AIR_GROUPS[i]: None for i in range(len(AIR_TO_AIR_GROUPS))
        }
        self.air_to_surface_groups = {
            AIR_TO_SURFACE_GROUPS[i]: {} for i in range(len(AIR_TO_SURFACE_GROUPS))
        }
        self.air_to_surface_group_name_to_guid = {
            AIR_TO_SURFACE_GROUPS[i]: None for i in range(len(AIR_TO_SURFACE_GROUPS))
        }
        self.electronic_jammer_groups = {
            ELECTRONIC_JAMMER_GROUPS[i]: {}
            for i in range(len(ELECTRONIC_JAMMER_GROUPS))
        }
        self.electronic_jammer_group_name_to_guid = {
            ELECTRONIC_JAMMER_GROUPS[i]: None
            for i in range(len(ELECTRONIC_JAMMER_GROUPS))
        }
        ...
        self.mid_level_avail_action_action_mask = {
            agent_id: {"action_mask": None, "avail_actions": None}
            for agent_id in MID_LEVEL_AGENT_IDs
        }
        self.high_level_avail_action_action_mask = {
            agent_id: {"action_mask": None, "avail_actions": None}
            for agent_id in HIGH_LEVEL_AGENT_IDs
        }
        self.auto_attack_ships = {}
        self.hl_last_attack_points = {
            hl_agent_id: None for hl_agent_id in HIGH_LEVEL_AGENT_IDs
        }
    ...
    def _update(self, scenario):
        ...
        self.operation_groups_class = {
            v.strName: v for _, v in self.side.groups.items()
        }
        self.operation_groups_class.update(
            {v.strName: v for _, v in self.side.aircrafts.items() if "EJ" in v.strName}
        )
        self._update_operation_groups()
        self._update_operation_groups_status()

        self.attack_ships = {
            k: v for k, v in self.side.contacts.items() if v.m_ContactType == 2
        }
        if self.steps % 10 == 0:
            self.update_ships_no_nav_area()
        self.attack_airs = {
            k: v for k, v in self.side.contacts.items() if v.m_ContactType == 0
        }
    ...
    def reset(self):
        ...
        self.rule_bot()
        self._set_simulate_speed(
            etc.SIMULATE_COMPRESSION_2, etc.DURATION_INTERVAL_2
        )  # 设置推演倍速为5倍速
    ...
    def get_bear_point(self, point_name_prefix, lat, lon, bearing_list, distance):
        ...
        for i, bearing in enumerate(bearing_list):
            ...
            cmd = (
                "print(World_GetPointFromBearing({"
    ...
    def delete_ref_points(self, points_name):
        for point_name in points_name:
            cmd = (
                ...
                + f"side='{self.side_name}',name='{point_name}'"
                + "})"
            )
            self.scenario.mozi_server.send_and_recv(cmd)

    def update_ref_points(
        self, area_name, latitude, longitude, outer_distance=20, inner_distance=19.8
    ):
        # 根据新的中心坐标点更新环形区域
        ...
        outer_points_dict = self.get_bear_point(
            point_name_prefix, latitude, longitude, bearing_list, outer_distance
        )
        ...
        inner_points_dict = self.get_bear_point(
            point_name_prefix, latitude, longitude, bearing_list, inner_distance
        )
    ...
    def set_no_nav_area(
        ...
        outer_points_dict = self.get_bear_point(
            point_name_prefix, center_lat, center_lon, bearing_list, outer_distance
        )
        ...
        inner_points_dict = self.get_bear_point(
            point_name_prefix, center_lat, center_lon, bearing_list, inner_distance
        )
        ...
        cmd = (
            f"ScenEdit_AddZone('{self.side_name}', 0, "
            + "{"
            + f"description='{area_center_name}', "
            + "Isactive=true, Affects={'Aircraft'}, Area={"
        )
    ...
    def update_ships_no_nav_area(self):
        for ship_name, ship_value in self.attack_ships.items():
            if ship_name in self.no_nav_area_info:
                self.update_ref_points(
                    ship_name, ship_value.dLatitude, ship_value.dLongitude
                )
            else:
                self.set_no_nav_area(
                    ship_name, ship_value.dLatitude, ship_value.dLongitude
                )
        # 删除多余禁航区参考点
        points_name = []
    ...
    def _high_level_mask(self, last_area_coord_points, hl_action, ret_hl):
        ...
        if delta_x + delta_y > 0:
            lower_index = (
                chain_index * ATTACK_CHAIN_POINT_NUMS if chain_index > 0 else 0
            )
    ...
    def _mid_level_mask(self, last_area_coord_points, hl_action, ml_avail_action):
        ...
        ml_avail_action = self.mid_level_agent_move_mask(
            last_area_coord_points, ml_avail_action, attack_point
        )
    ...
    def _action_mask(self, action_dict, ret_hl):
        for (
            op_group_name,
            last_area_coord_points,
        ) in self.operation_groups_last_area.items():
            ...
            ml_avail_action = self._mid_level_mask(
                last_area_coord_points, hl_action, ml_avail_action
            )
            ...
            for aac in ml_avail_action:
                ...
                ml_action_embed[int(aac)] = self.mid_level_action_embed[ml_agent_id][
                    int(aac)
                ]
            self.mid_level_avail_action_action_mask[ml_agent_id][
                "action_mask"
            ] = ml_action_mask
            self.mid_level_avail_action_action_mask[ml_agent_id][
                "avail_actions"
        ...
        for hl_agent_id in HIGH_LEVEL_AGENT_IDs:
            for ml_agent_id in HL_AGENTS_TO_ML_AGENTS[hl_agent_id]:
                ...
                chain_index, hl_avail_action = self._high_level_mask(
                    last_area_coord_points, hl_action, ret_hl
                )
        ...
        for hl_agent_id in HIGH_LEVEL_AGENT_IDs:
            ...
            for aac in temp_avail_action:
                ...
                hl_action_embed[int(aac)] = self.high_level_action_embed[hl_agent_id][
                    int(aac)
                ]
            self.high_level_avail_action_action_mask[hl_agent_id][
                "action_mask"
            ] = hl_action_mask
            self.high_level_avail_action_action_mask[hl_agent_id][
                "avail_actions"
    ...
    def update_ewr_location(self):
        ...
        if self.ewr_last_point:
            ...
            current_point_detect_status = self.detect_point_distance(
                self.ewr_last_point, EWR_DETECT_DISTANCE
            )
            ewr_unit = [
                v for k, v in self.rule_operation_units.items() if v.m_Type == 4002
            ]
            if not ewr_unit:
                return
            # 向后退
            ...
            # 前进
                ...
                if duration_time / 60.0 > MODEL_START_TIME:
                    if point_index == 4:
                        ...
                        next_point_detect_status = self.detect_point_distance(
                            EWR_PATROL_AREA[point_index + 1], EWR_DETECT_DISTANCE
                        )
                        if next_point_detect_status:
                            pass
                        else:
                            self.ewr_last_point = EWR_PATROL_AREA[point_index + 1]
                            self.set_ewr_location(
                                self.ewr_last_point, status_code="update"
                            )
    ...
    def set_ewr_location(self, point, status_code=None):
        ...
        ewr_unit = {
            k: v for k, v in self.rule_operation_units.items() if v.m_Type == 4002
        }
        assert ewr_unit is not {}
        if status_code is "init":
            self.unit_out_to_next_point(
                list(ewr_unit.keys())[0], list(ewr_unit.values())[0].strName, point
            )
            self.ewr_last_point = point
        elif status_code is "update":
            lat, lon = convert_coordinate_to_lat_lon(
                OPERATION_AREA_GRID_DIM, RP_1, RP_2, point
            )
            self.control_operation_group_next_point(
                list(ewr_unit.values())[0].strName, lat, lon
            )
            self.ewr_last_point = point
    ...
    def rule_bot(self):
        ...
        for k, v in self.rule_operation_units.items():
            if v.strName in [TOP_RULE_UNITS_NAME[0], BOT_RULE_UNITS_NAME[0]]:
                RULE_OPERATION_UNITS_TASK_STATUS[v.strName]["first_stage"][
                    "status"
                ] = True
                self.unit_out_to_next_point(
                    k,
        ...
        scout_rule_unit_survival_status = {
            scout_units_name[i]: True for i in range(len(scout_units_name))
        }
        while True:
            self.scenario = self.env.step()  # 墨子环境step
            self._update(self.scenario)
            rule_operation_units_name_to_guid = {
                v.strName: k for k, v in self.rule_operation_units.items()
            }
            duration_time = self.m_Time - self.m_StartTime
            # 更新判断执行侦察任务的单元存活状态
            ...
            if duration_time / 60.0 >= MODEL_START_TIME:
                ...
                for (
                    scout_rule_unit_name,
                    survival_status,
                ) in scout_rule_unit_survival_status.items():
                    if survival_status:
                        guid = rule_operation_units_name_to_guid[scout_rule_unit_name]
                        if (
                            self.rule_operation_units[guid].strAirOpsConditionString
                            == 1
                        ):  # 飞机处于停泊状态
                            self.unit_out_to_next_point(
                                guid, scout_rule_unit_name, SCOUT_POINT_LIST[4]
                            )
                        else:
                            lat, lon = convert_coordinate_to_lat_lon(
                                OPERATION_AREA_GRID_DIM, RP_1, RP_2, SCOUT_POINT_LIST[4]
                            )
                            self.control_operation_group_next_point(
                                scout_rule_unit_name, lat, lon
                            )
                break

            # 更新判断四个侦察点附近是否存在敌方单元
            detect_status = {
                SCOUT_POINT_LIST[i]: self.detect_point_distance(
                    SCOUT_POINT_LIST[i], SCOUT_DETECT_DISTANCE
                )
            ...
            for (
                ...
                if survival_status:
                    # 是否需要规避进攻
                    self.avoid_attack(
                        rule_operation_units_name_to_guid[scout_rule_unit_name],
                        scout_rule_unit_name,
                    )
                    ...
                    if scout_rule_unit_name in first_wave_scout_units_name:
                        # 如果第二阶段任务已启动
                        if RULE_OPERATION_UNITS_TASK_STATUS[scout_rule_unit_name][
                            "second_stage"
                        ]["status"]:
                            pass
                        # 如果第二阶段任务未启动，择时启动第二阶段任务
                            ...
                            if duration_time / 60.0 >= FIRST_STAGE_DURATION_TIME:
                                ...
                                ss_detect_status = detect_status[
                                    RULE_OPERATION_UNITS_TASK_STATUS[
                                        scout_rule_unit_name
                                    ]["second_stage"]["point"]
                                ]
                    ...
                    # 第二波侦察单元
                        ...
                        # 第一阶段任务未启动
                            ...
                            if scout_rule_unit_name in TOP_RULE_UNITS_NAME:
                                ...
                                guid = rule_operation_units_name_to_guid[
                                    scout_rule_unit_name
                                ]
                                if not scout_rule_unit_survival_status[
                                    TOP_RULE_UNITS_NAME[0]
                                ]:
                                    RULE_OPERATION_UNITS_TASK_STATUS[
                                        scout_rule_unit_name
                                    ]["first_stage"]["status"] = True
                                    self.unit_out_to_next_point(
                                        guid,
                                        scout_rule_unit_name,
                                        RULE_OPERATION_UNITS_TASK_STATUS[
                                    ...
                                    guid_2 = rule_operation_units_name_to_guid[
                                        TOP_RULE_UNITS_NAME[0]
                                    ]
                                    if self.rule_operation_units[
                                        guid_2
                                    ].strAirOpsConditionString in [
                                        7,
                                        11,
                                    ]:  # 飞机处于返航状态
                                        RULE_OPERATION_UNITS_TASK_STATUS[
    ...
    def avoid_attack(self, unit_guid, unit_name):
        ...
        detect_status = self.compute_point_distance(
            unit.dLongitude, unit.dLatitude, AVOID_ATTACK_DISTANCE
        )
        # 附近30公里存在敌方单元，启动规避
        if detect_status:
            for _ in range(EVADE_NUMS):
                ...
                self.control_operation_unit(
                    unit_name, altitude * 20000, speed * 1000, heading * 360
                )
    ...
    def all_unit_avoid_attack(self):
        ...
        for _, unit_class in ou.items():
            detect_status = self.compute_point_distance(
                unit_class.dLongitude, unit_class.dLatitude, AVOID_ATTACK_DISTANCE
            )
            if detect_status:
        ...
        if t:
            for _ in range(EVADE_NUMS):
                for name in t:
                    ...
                    self.control_operation_unit(
                        name, altitude * 20000, speed * 1000, heading * 360
                    )
    ...
    def _set_side_doctrine(self):
        ...
        doctrine.set_weapon_control_status(
            domain="weapon_control_status_surface", fire_status="0"
        )
        # 电磁管控
    ...
    def modify_unit_doctrine(unit_class, radar_status, fire_status=False):
        ...
        if fire_status:
            doctrine.set_weapon_control_status(
                domain="weapon_control_status_air", fire_status="0"
            )
    ...
    def _generate_operation_group_features(self, operation_group_name):
        ...
        for unit_guid, unit_value in in_group_units.items():
            for contact_guid, contact_value in contacts.items():
                dis = get_two_point_distance(
                    unit_value.dLongitude,
                    unit_value.dLatitude,
                    contact_value.dLongitude,
                    contact_value.dLatitude,
                )
        ...
        for unit_guid, unit_value in in_group_units.items():
            for local_unit_guid, local_unit_value in self.agent_operation_units.items():
                dis = get_two_point_distance(
                    unit_value.dLongitude,
                    unit_value.dLatitude,
                    local_unit_value.dLongitude,
                    local_unit_value.dLatitude,
                )
    ...
    def _get_contact_area(contact_unit):
        ...
        if contact_unit.m_UncertaintyArea:
            uncertainty_area = list(
                map(lambda x: x.split("$"), contact_unit.m_UncertaintyArea.split("$0@"))
            )
            ua = []
    ...
    def _get_unit_weapon(unit):
        ...
        weapon_list = list(
            map(
                lambda x, y: x + [y[-1]],
                list(map(lambda x: x[0].split("x "), weapon)),
                weapon,
            )
        )
        return weapon_list
            ...
            if self.reset_nums % 5 == 0:
                ...
                for _ in range(MAX_DOCKER_RETRIES):
                    ...
                    try:
                        ...
                        print(
                            f"{time.strftime('%H:%M:%S')} 在第{self.steps}步，第{_}次重启docker失败！！！"
                        )
                        continue
                print("开始mozi reset!!!")
                self.scenario = self.env.reset()
    ...
    def update_hl_agent_reward(self, action_dict, is_done):
        ...
        for hl_agent_id in HIGH_LEVEL_AGENT_IDs:
            ...
            if anchor_attack_point_index:
                self.hl_reward[hl_agent_id] -= (
                    abs(attack_point_index - anchor_attack_point_index) / reward_bound
                )
                # chain_index = int(anchor_attack_point_index / ATTACK_CHAIN_POINT_NUMS)
                # if 0 < chain_index < ATTACK_CHAINS - 1:
    ...
    def get_operation_groups_init_area(self):
        for operation_group_name in OPERATION_GROUPS:
            ...
            coordinate_points = get_cell(
                OPERATION_AREA_GRID_DIM, RP_1, RP_2, (group_lat, group_lon)
            )
    ...
    def compute_next_point(self, next_area_index, operation_group_name):
        ...
        if last_area_coord_points:
            ...
            if next_area[str(next_area_index)]:
                # 更新作战编队最新区域
                self.operation_groups_last_area[operation_group_name] = next_area[
                    str(next_area_index)
                ]
                coordinate_index = next_area[str(next_area_index)]
                if "A2A" in operation_group_name:
                    lat, lon = convert_coordinate_to_lat_lon(
                        OPERATION_AREA_GRID_DIM, RP_1, RP_2, coordinate_index[2]
                    )
                    return lat, lon, coordinate_index[2], last_area_coord_points
                elif "A2S" in operation_group_name:
                    lat, lon = convert_coordinate_to_lat_lon(
                        OPERATION_AREA_GRID_DIM, RP_1, RP_2, coordinate_index[0]
                    )
    ...
    def control_operation_group_space(self, action_dict):
        ...
        for agent_id, ac in temp_ac_dict.items():
            ...
            if next_area_index:  # 0 为保持不动
                lat, lon, ci, last_cp = self.compute_next_point(
                    next_area_index, group_name
                )
                # 控制作战编队去下一个点
                ...
                for ship_guid, ship_value in self.attack_ships.items():
                    ...
                    for unit_guid, unit_value in in_group_units.items():
                        dis = get_two_point_distance(
                            unit_value.dLongitude,
                            unit_value.dLatitude,
                            ship_value.dLongitude,
                            ship_value.dLatitude,
                        )
                        distance += dis
        ...
        if "A2S" in group_name:
            ...
            if ret and target_guids:
                ...
                self.auto_attack_target(
                    self.operation_groups_name_to_guid[group_name], target_guids
                )
    ...
    def auto_attack_target(self, ops_group_guid, attack_guid_list):
        for attack_guid in attack_guid_list:
            cmd = (
                f"ScenEdit_AttackContact('{ops_group_guid}', '{attack_guid}',"
                + "{mode = 0})"
            )
            ret = self.scenario.mozi_server.send_and_recv(cmd)
    ...
    def create_operation_groups(self):
        ...
        for op_unit_key, op_unit_value in self.agent_operation_units.items():
            ...
            if AIR_TO_SURFACE_GROUP[
                "UNIT_TYPE"
            ] in op_unit_value.strName and self._get_weapon_info(
                weapon_list, AIR_TO_SURFACE_GROUP["WEAPON_TYPE"]
            ):
                air_to_surface[op_unit_key] = op_unit_value
            if (
                AIR_TO_AIR_GROUP["UNIT_TYPE"] in op_unit_value.strName
                and self._get_weapon_info(weapon_list, AIR_TO_AIR_GROUP["WEAPON_TYPE"])
                and not self._get_weapon_info(
                    weapon_list, AIR_TO_SURFACE_GROUP["WEAPON_TYPE"]
                )
            ):
        ...
        assert (
            air_to_air.__len__()
            >= self.air_to_air_groups.__len__() * AIR_TO_AIR_GROUP["NUM"]
        )
        ...
        assert (
            air_to_surface.__len__()
            >= self.air_to_surface_groups.__len__() * AIR_TO_SURFACE_GROUP["NUM"]
        )
        ...
        assert (
            electronic_jammer.__len__()
            >= self.electronic_jammer_groups.__len__() * ELECTRONIC_JAMMER["NUM"]
        )
    ...
    def operation_group_name_to_guid(self):
        ...
        for k, v in self.side.groups.items():
            for group_name, units in self.air_to_air_groups.items():
                if v.m_GroupLead in units.keys():
                    ...
                    cmd = (
                        "ScenEdit_SetUnit({"
                        + f"Unitname='{v.strName}',"
                        + f"Newname='{group_name}'"
                        + "})"
                    )
                    self.scenario.mozi_server.send_and_recv(cmd)
                    break
        ...
        for group_name, unit in self.electronic_jammer_groups.items():
            ...
            cmd = (
                "ScenEdit_SetUnit({"
                + f"Unitname='{value.strName}',"
                + f"Newname='{group_name}'"
                + "})"
            )
            self.scenario.mozi_server.send_and_recv(cmd)

        self.operation_groups = dict(
            self.air_to_air_groups,
            **self.air_to_surface_groups,
            **self.electronic_jammer_groups,
        )
        self.operation_groups_name_to_guid = dict(
            self.air_to_air_group_name_to_guid,
            **self.air_to_surface_group_name_to_guid,
            **self.electronic_jammer_group_name_to_guid,
        )
...
class HighLevelEnv(MultiAgentEnv):
    ...
    def step(self, action_dict):
        ...
        if self.high_level_steps == 1:
            ...
            (
                ...
                info,
                ml_avail_action_action_mask,
                hl_avail_action_action_mask,
            ) = self.mozi_env.step(
                multi_agent_action_dict,
                self.update_next_point,
                self.update_next_attack_point,
            )
            self.update_next_point = False
            self.update_next_attack_point = False
            ...
            for low_level_agent_id, obs in low_level_agent_obs.items():
                ...
                if not ret_ml["__all__"]:
                    ...
                    t_low_level_agent_obs[low_level_agent_id][SampleBatch.OBS].append(
                        mid_level_action
                    )
                    t_low_level_agent_obs[low_level_agent_id][SampleBatch.OBS].append(
                        high_level_action
                    ...
                    LL_DATA_COLLECTOR[low_level_agent_id].add_values(
                        low_level_values_dict
                    )
                else:
                    # 把底层智能体的obs添加进数据存储单元
                    ...
                    LAST_ML_AGENTS_OBS_REWARD_DONE[ml_agent_id][
                        ENV_STATE
                    ] = temp_ml_agent_state

                    temp_ml_agent_obs = []
                    for o in LL_DATA_COLLECTOR[low_level_agent_id].buffers[
                        SampleBatch.OBS
                    ]:
            ...
            if ret_ml["__all__"]:
                ...
                if not ret_hl["__all__"]:
                    ...
                    for ml_agent_id in MID_LEVEL_AGENT_IDs:
                        ...
                        ml_agent_obs[ml_agent_id][
                            ENV_STATE
                        ] = LAST_ML_AGENTS_OBS_REWARD_DONE[ml_agent_id][ENV_STATE]
                        ml_agent_reward[ml_agent_id] = LAST_ML_AGENTS_OBS_REWARD_DONE[
                            ml_agent_id
                        ][SampleBatch.REWARDS]
                        ml_agent_done[ml_agent_id] = LAST_ML_AGENTS_OBS_REWARD_DONE[
                            ml_agent_id
                        ][SampleBatch.DONES]
                ...
                for hl_agent_id in HIGH_LEVEL_AGENT_IDs:
                    temp_hl_agent_obs = []
                    for ml_agent_id in HL_AGENTS_TO_ML_AGENTS[hl_agent_id]:
                        temp_hl_agent_obs += LAST_ML_AGENTS_OBS_REWARD_DONE[
                            ml_agent_id
                        ][SampleBatch.OBS]

                    high_level_values_dict = {
                        SampleBatch.OBS: temp_hl_agent_obs,
            ...
            if ret_hl["__all__"]:
                hl_agent_obs = {
                    hl_agent_id: {SampleBatch.OBS: [], ENV_STATE: []}
                    for hl_agent_id in HIGH_LEVEL_AGENT_IDs
                }
                hl_agent_reward = hl_reward
                hl_agent_done = {
                    hl_agent_id: False for hl_agent_id in HIGH_LEVEL_AGENT_IDs
                }
                hl_agent_info = {
                    hl_agent_id: {} for hl_agent_id in HIGH_LEVEL_AGENT_IDs
                }
                ...
                for hl_agent_id in HIGH_LEVEL_AGENT_IDs:
                    ...
                    hl_agent_obs[hl_agent_id][
                        "action_mask"
                    ] = hl_avail_action_action_mask[hl_agent_id]["action_mask"]
                    hl_agent_obs[hl_agent_id][
                        "avail_actions"
                    ] = hl_avail_action_action_mask[hl_agent_id]["avail_actions"]

                    hl_agent_done[hl_agent_id] = LAST_ML_AGENTS_OBS_REWARD_DONE[
                        "ml_agent_0"
                    ][SampleBatch.DONES]
        ...
        # 返回给ray，下一步ray只返回中层智能体的动作。
            ...
            for ml_agent_id in MID_LEVEL_AGENT_IDs:
                ...
                ml_agent_obs[ml_agent_id][ENV_STATE] = LAST_ML_AGENTS_OBS_REWARD_DONE[
                    ml_agent_id
                ][ENV_STATE]
        ...
        # 把中层智能体的动作，拼接到底层智能体的状态空间，返回给ray，下一步ray只返回底层智能体的动作。
            ...
            for ml_agent_id, ac in mid_level_action_dict.items():
                ...
                ll_agent_obs[ll_agent_id][SampleBatch.OBS] = LL_DATA_COLLECTOR[
                    ll_agent_id
                ].buffers[SampleBatch.OBS][-1]
                # 用新的上层和中层的动作替换掉底层智能体最近一次的obs中的旧的上层和中层的动作
                ll_agent_obs[ll_agent_id][SampleBatch.OBS][
                    -1
                ] = LAST_HL_ML_AGENTS_ACTION[hl_agent_id]
                ll_agent_obs[ll_agent_id][SampleBatch.OBS][-2] = ac

                ll_agent_obs[ll_agent_id][ENV_STATE] = LL_DATA_COLLECTOR[
                    ll_agent_id
                ].buffers[ENV_STATE][-1]
                ll_agent_reward[ll_agent_id] = LL_DATA_COLLECTOR[ll_agent_id].buffers[
                    SampleBatch.REWARDS
                ][-1]
                ll_agent_done[ll_agent_id] = LL_DATA_COLLECTOR[ll_agent_id].buffers[
                    SampleBatch.DONES
                ][-1]
                ...
                LL_DATA_COLLECTOR[ll_agent_id].add_init_obs(
                    ll_agent_obs[ll_agent_id][ENV_STATE],
    ...
    def reset(self):
        ...
        HL_DATA_COLLECTOR = {
            agent_id: AgentCollector() for agent_id in HIGH_LEVEL_AGENT_IDs
        }

        low_level_agent_obs, global_obs = self.mozi_env.reset()
        hl_agent_obs = {
            hl_agent_id: {SampleBatch.OBS: [], ENV_STATE: global_obs}
            for hl_agent_id in HIGH_LEVEL_AGENT_IDs
        }
        ml_agent_obs = {
            ml_agent_id: {SampleBatch.OBS: None, ENV_STATE: global_obs}
            for ml_agent_id in MID_LEVEL_AGENT_IDs
        }
        ...
        for low_level_agent_id, obs in low_level_agent_obs.items():
            ...
            t_obs[SampleBatch.OBS].append(mid_level_action)
            t_obs[SampleBatch.OBS].append(high_level_action)
            LL_DATA_COLLECTOR[low_level_agent_id].add_init_obs(
                global_obs, self.high_level_steps, ret_ml, t_obs[SampleBatch.OBS]
            )

            low_level_agent_obs[low_level_agent_id][SampleBatch.OBS].append(
                mid_level_action
            )
            low_level_agent_obs[low_level_agent_id][SampleBatch.OBS].append(
                high_level_action
            ...
            ml_agent_obs[ml_agent_id]["avail_actions"] = self.mid_level_action_embed[
                ml_agent_id
            ]
