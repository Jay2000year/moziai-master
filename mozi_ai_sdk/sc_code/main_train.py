...
from mozi_ai_sdk.sc_code_orig.envs.env_sc_new import (
    SCEnv,
    LL_AGENTS_TO_OPERATION_GROUPS,
    LOW_LEVEL_OBS_SPACE,
    STATE_SPACE,
    MID_LEVEL_OBS_SPACE,
    MID_LEVEL_STATE_SPACE,
    HIGH_LEVEL_OBS_SPACE,
    HIGH_LEVEL_STATE_SPACE,
    LOW_LEVEL_ACT_SPACE,
    MID_LEVEL_ACT_SPACE,
    MID_LEVEL_ACTION_EMBED,
    HIGH_LEVEL_ACT_SPACE,
    HIGH_LEVEL_ACTION_EMBED,
)
...
parser.add_argument(
    "--mozi_server_path", type=str, default="D:\\mozi_server\\Mozi\\MoziServer\\bin"
)
...
def hierarchical_training_workflow(workers: WorkerSet, config: dict):
    ...
    def add_hl_qmix_metrics(batch):
        print(
            "high level QMIX policy learning on samples from",
            batch.policy_batches.keys(),
            "env steps",
            batch.env_steps(),
            "agent steps",
    ...
    def add_ml_qmix_metrics(batch):
        print(
            "mid level QMIX policy learning on samples from",
            batch.policy_batches.keys(),
            "env steps",
            batch.env_steps(),
            "agent steps",
    ...
    hl_qmix_store_op = r1.for_each(
        SelectExperiences(["high_level_qmix_policy"])
    ).for_each(StoreToReplayBuffer(local_buffer=hl_replay_buffer))
    hl_qmix_replay_op = (
        Replay(local_buffer=hl_replay_buffer)
        ...
        .for_each(
            UpdateTargetNetwork(workers, 500, policies=["high_level_qmix_policy"])
        )
    )
    ...
    ml_qmix_store_op = r2.for_each(
        SelectExperiences(["mid_level_qmix_policy"])
    ).for_each(StoreToReplayBuffer(local_buffer=ml_replay_buffer))
    ml_qmix_replay_op = (
        Replay(local_buffer=ml_replay_buffer)
        ...
        .for_each(UpdateTargetNetwork(workers, 500, policies=["mid_level_qmix_policy"]))
    )
    ...
    train_op = Concurrently(
        [hl_qmix_train_op, ml_qmix_train_op, low_level_ppo_train_op],
        mode="async",
...
if __name__ == "__main__":
    ...
    ppo_config = Trainer.merge_trainer_configs(
        PPO_CONFIG, ppo_extra_config, _allow_unknown_configs=True
    )
    ...
    high_level_qmix_obs_space = Tuple(
        [
            Dict(
                {
                    "obs": Box(
                        float("-inf"), float("inf"), shape=(HIGH_LEVEL_OBS_SPACE,)
                    ),
                    ENV_STATE: Box(
                        float("-inf"), float("inf"), shape=(HIGH_LEVEL_STATE_SPACE,)
                    ),
                    # 上层智能体需要新增action_mask和avail_action
                    "action_mask": Box(0, 1, shape=(HIGH_LEVEL_ACT_SPACE,)),
                    "avail_actions": Box(
                        -100, 100, shape=(HIGH_LEVEL_ACT_SPACE, HIGH_LEVEL_ACTION_EMBED)
                    ),
                }
            ),
            Dict(
    ...
    mid_level_qmix_obs_space = Tuple(
        [
            Dict(
                {
                    "obs": Box(
                        float("-inf"), float("inf"), shape=(MID_LEVEL_OBS_SPACE,)
                    ),
                    ENV_STATE: Box(
                        float("-inf"), float("inf"), shape=(MID_LEVEL_STATE_SPACE,)
                    ),
                    # 中层智能体需要新增action_mask和avail_action
                    "action_mask": Box(0, 1, shape=(MID_LEVEL_ACT_SPACE,)),
                    "avail_actions": Box(
                        -10, 10, shape=(MID_LEVEL_ACT_SPACE, MID_LEVEL_ACTION_EMBED)
                    ),
                }
            ),
            Dict(
    ...
    low_level_ppo_act_space = Tuple(
        [Discrete(LOW_LEVEL_ACT_SPACE[0]), Discrete(LOW_LEVEL_ACT_SPACE[1])]
    )

    policies = {
        "high_level_qmix_policy": (
            QMixTorchPolicy,
            high_level_qmix_obs_space,
            high_level_qmix_act_space,
            QMIX_CONFIG,
        ),
        "mid_level_qmix_policy": (
            QMixTorchPolicy,
            mid_level_qmix_obs_space,
            mid_level_qmix_act_space,
            QMIX_CONFIG,
        ),
        "low_level_ppo_policy_a2a": (
            PPOTorchPolicy,
            low_level_ppo_obs_space,
            low_level_ppo_act_space,
            ppo_config,
        ),
        "low_level_ppo_policy_a2s": (
            PPOTorchPolicy,
