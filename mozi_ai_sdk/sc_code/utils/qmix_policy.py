...
class QMixLoss(nn.Module):
    ...
    def forward(
        ...
        # mac_out = _unroll_mac(self.bcmodel, obs)
        # by dixit
        ...
        # by dixit
        if has_avail_actions:
            target_mac_out = _temp_unroll_mac(
                self.target_model, next_obs, next_avail_actions
            )
        else:
            target_mac_out = _unroll_mac(self.target_model, next_obs)
        ...
        if self.double_q:
            ...
            # using next_obs
            # mac_out_tp1 = _unroll_mac(self.bcmodel, next_obs)
            # by dixit
            if has_avail_actions:
...
class QMixTorchPolicy(Policy):
    ...
    def __init__(self, obs_space, action_space, config):
        ...
        self.h_size = config["bcmodel"]["lstm_cell_size"]
        ...
        if isinstance(agent_obs_space, Dict):
            ...
            config["bcmodel"]["full_obs_space"] = agent_obs_space
    ...
    def learn_on_batch(self, samples):
        ...
        (
            ...
            env_global_state,
            avail_actions,
        ) = self._unpack_observation(samples[SampleBatch.CUR_OBS])
        (
            next_obs_batch,
            next_action_mask,
        ...
        output_list, _, seq_lens = chop_into_sequences(
            ...
            max_seq_len=self.config["bcmodel"]["max_seq_len"],
            dynamic_max=True,
        )
        ...
        # by dixit
        if self.has_avail_actions:
            action_embed = self.observation_space.original_space[0][
                "avail_actions"
            ].shape[1]
    ...
    def get_weights(self):
        return {
            "bcmodel": self._cpu_dict(self.model.state_dict()),
    ...
    def set_weights(self, weights):
        self.model.load_state_dict(self._device_dict(weights["bcmodel"]))
    ...
    def _unpack_observation(self, obs_batch):
        ...
        if self.has_avail_actions:
            ...
            avail_actions = np.concatenate(
                [o["avail_actions"] for o in unpacked], axis=1
            ).reshape([len(obs_batch), self.n_agents, self.n_actions, action_embed])
        else:
