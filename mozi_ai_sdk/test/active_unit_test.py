...
class TestActiveUnit(TestFramework):
    ...
    def test_get_weapon_infos(self):
        ...
        self.assertTrue(
            ["48x MIM-104B型“爱国者-1”防空导弹", "hsfw-dataweapon-00000000001152"]
            in weapon_infos
        )
        self.assertTrue(
            ["6x FIM-92B型“毒刺”防空导弹[POST]", "hsfw-dataweapon-00000000000735"]
            in weapon_infos
        )
    ...
    def test_get_range_to_contact(self):
        """获取与目标的距离"""
        distance = self.ground_to_air_missile_squadron.get_range_to_contact(
            self.enemy_airplane_guid
        )
    ...
    def test_plot_course(self):
        ...
        # 反潜机1
        course_list = [
            (26.0728267704942, 125.582813973341),
            (26.410343165174, 125.857575579442),
        ]
    ...
    def test_allocate_weapon_to_target_manual(self):
        ...
        self.ground_to_air_missile_squadron.allocate_weapon_to_target(
            self.enemy_airplane_guid, weapon_guid, 1
        )
    ...
    def test_unit_drop_target_contact(self):
        ...
        # 设置态势显示-目标瞄准矢量：所有单元
        # 单元放弃目标
        self.ground_to_air_missile_squadron.unit_drop_target_contact(
            self.enemy_airplane_guid
        )
    ...
    def test_ignore_plotted_course_when_attacking(self):
        ...
        self.assertEqual(
            1,
            self.ground_to_air_missile_squadron.get_doctrine().m_IgnorePlottedCourseWhenAttacking,
        )
        # 设置攻击时不忽略计划航线
        ...
        self.ground_to_air_missile_squadron.ignore_plotted_course_when_attacking(
            "Inherited"
        )
        self.env.step()
        self.assertEqual(
    ...
    def test_return_to_base(self):
        ...
        self.env.step()
        self.assertEqual(
            self.antisubmarine_aircraft.strActiveUnitStatus, "状态: 返回基地(受命)"
        )
    ...
    def test_select_new_base(self):
        ...
        self.env.step()
        self.assertEqual(
            self.antisubmarine_aircraft.m_HostActiveUnit, self.airport_2.strGuid
        )
    ...
    def test_assign_unit_to_mission(self):
        ...
        self.env.step()
        self.assertEqual(
            self.red_side.get_missions_by_name(missing_name).strGuid,
            self.docked_f35_1.m_AssignedMission,
        )
    ...
    def test_assign_unit_to_mission_docked_ship(self):
        ...
        self.assertEqual(
            self.red_side.get_missions_by_name(missing_name).strGuid,
            self.docked_ship_1.m_AssignedMission,
        )
    ...
    def test_manual_attack(self):
        ...
        weapon_guid = "hsfw-dataweapon-00000000001152"
        # 向敌机1发射一枚 MIM-104B型“爱国者-1”防空导弹
        self.ground_to_air_missile_squadron.manual_attack(
            self.enemy_airplane_guid, weapon_guid, 1
        )
        # 查看墨子服务端, "地空导弹中队"发射了一枚 MIM-104B型“爱国者-1”防空导弹
        count = 0
    ...
    def test_set_loadout(self):
        # 包含可选武器
        self.docked_f35_1.set_loadout(
            loadoutId=2995,
            time_to_ready_minutes=10,
    ...
    def test_set_magazine_weapon_current_load(self):
        ...
        self.ground_to_air_missile_squadron.set_magazine_weapon_current_load(
            weapon_record_guid, 10
        )
        self.env.step()
    ...
    def test_set_weapon_current_load(self):
        ...
        self.ground_to_air_missile_squadron.set_weapon_current_load(
            weapon_record_guid, 1
        )
        self.env.step()
    ...
    def test_set_weapon_reload_priority(self):
        ...
        for k, v in mounts.items():
            if v.strName == "M901式爱国者导弹":
                ...
                break
        weapon_record_guid = mount.m_LoadRatio.split("$")[0]
        self.ground_to_air_missile_squadron.set_weapon_reload_priority(
            weapon_record_guid, "true"
        )
        self.env.step()
        self.ground_to_air_missile_squadron.set_weapon_reload_priority(
            weapon_record_guid, "false"
        )
        self.env.step()
    ...
    def test_add_weapon_to_unit_magazine(self):
        ...
        self.ground_to_air_missile_squadron.add_weapon_to_unit_magazine(
            magazine_guid, weapon_db_guid, 3
        )
        self.env.step()
    ...
    def test_allocate_all_weapons_to_target(self):
        """为手动交战分配同类型所有武器。"""
        self.ground_to_air_missile_squadron.allocate_all_weapons_to_target(
            self.enemy_airplane_guid, 1152
        )
    ...
    def test_remove_salvo_target(self):
        ...
        """取消手动交战时齐射攻击目标"""
        salvo_guid = self.ground_to_air_missile_squadron.allocate_salvo_to_target(
            self.enemy_airplane_guid, 1152
        )
        self.env.step()
    ...
    def test_allocate_weapon_to_target(self):
        ...
        # 手动攻击
        self.ground_to_air_missile_squadron.allocate_weapon_to_target(
            self.enemy_airplane_guid, "hsfw-dataweapon-00000000001152", 1
        )
        count = 0
        while True:
            self.env.step()
    ...
    def test_allocate_weapon_auto_targeted(self):
        ...
        # weapon_db_guid = 'hsfw-dataweapon-00000000001152'
        self.ground_to_air_missile_squadron.allocate_weapon_auto_targeted(
            target_guid_list, 1152, 3
        )
        count = 0
        while True:
            self.env.step()
    ...
    def test_update_way_point(self):
        ...
        self.antisubmarine_aircraft.plot_course(course_list)
        self.env.step()
        self.antisubmarine_aircraft.update_way_point(
            0, 27.0728267704942, 126.582813973341
        )
        self.env.step()
        way_points_info = self.antisubmarine_aircraft.get_way_points_info()
    ...
    def test_set_unit_damage(self):
        """设置单元毁伤值"""
        # 设置航空设施毁伤  --- 被摧毁
        component_guid = self.airport_1.m_AirFacilitiesComponent.split("$0$0$@")[
            0
        ].split("$")[0]
    ...
    def test_set_magazine_weapon_number(self):
        ...
        self.ground_to_air_missile_squadron.set_magazine_weapon_number(
            magazine_guid, weapon_db_guid, 2
        )
        self.env.step()
