...
class TestAirCraft(TestFramework):
    ...
    def test_quick_turnaround(self):
        ...
        self.env.step()
        self.assertEqual(
            self.antisubmarine_aircraft.strQuickTurnAroundInfo,
            "支持, 白天/夜间: 1 / 2波次, 0 / 4小时飞行时间, 无维修时间",
        )
    ...
    def test_ready_immediately(self):
        ...
        self.docked_f35_1.ready_immediately(
            loadout_dbid=2995,
            enable_quick_turnaround="true",
            combo_box=-1,
            immediately_go="true",
            optional_weapon="false",
            ignore_weapon="false",
        )
        self.env.step()
        self.env.step()
    ...
    def test_get_loadout(self):
        ...
        loadout_name = (
            "AGM-84C型“鱼叉”反舰导弹, “霍尼韦尔”Mk46 Mod 5 “尼尔蒂普”型鱼雷, AN/SSQ-53A型定向频率分析和记录被动"
    ...
    def test_refuel(self):
        ...
        # 距离受油机2较近，选择受油机2加油
        self.assertEqual(
            self.refuel_aircraft.strActiveUnitStatus,
            "状态: 正赶赴加油点 (加油对象: 加油机2) (机动到加油阵位)",
        )
    ...
    def test_refuel_manual(self):
        ...
        # 选择了受油机加油
        self.assertEqual(
            self.refuel_aircraft.strActiveUnitStatus, "状态: 正赶赴加油点 (加油对象: 加油机) (机动到加油阵位)"
        )
