...
class TestScenario(TestFramework):
    ...
    def test_add_trigger_unit_destroyed(self):
        ...
        target_filter_dict = {
            ...
            "TARGETTYPE": 1,
            "TARGETSUBTYPE": 6002,
            "SPECIFICUNITCLASS": 2802,
        }
        self.scenario.add_trigger_unit_destroyed(
            "P-3C型“猎户座” II反潜机被摧毁", target_filter_dict
        )
    ...
    def test_update_trigger_unit_destroyed(self):
        ...
        # 修改名称和目标
        target_filter_dict = {"TARGETSIDE": "红方", "TARGETTYPE": 1}
        self.scenario.update_trigger_unit_destroyed(
            "红方单元被摧毁", "飞机被摧毁", target_filter_dict
        )
        self.env.step()
        # 只修改名称
        ...
        target_filter_dict = {"SPECIFICUNIT": "反潜机2"}
        self.scenario.update_trigger_unit_destroyed(
            "飞机被摧毁2", target_filter_dict=target_filter_dict
        )
        self.env.step()
    ...
    def test_add_trigger_unit_damaged(self):
        ...
        self.scenario.add_trigger_unit_damaged(
            "P-3C型“猎户座” II反潜机被毁伤50%", target_filter_dict, 50
        )
    ...
    def test_update_trigger_unit_damaged(self):
        ...
        rename = "飞机被毁伤20%"
        damage_percent = 20
        self.scenario.update_trigger_unit_damaged(
            "红方单元被毁伤10%",
            rename=rename,
    ...
    def test_update_trigger_points(self):
        ...
        rename = "推演方得分触发器-改1"
        # 刚好抵达200分
        self.scenario.update_trigger_points(
            "推演方得分触发器", rename=rename, side="蓝方", point_value=200, reach_direction=1
        )
        self.env.step()
        rename = "推演方得分触发器-改2"
        # 低于200分
        self.scenario.update_trigger_points(
            "推演方得分触发器-改1", rename=rename, side="蓝方", point_value=200, reach_direction=2
        )
        self.env.step()
    ...
    def test_update_trigger_time(self):
        ...
        # 实际设置时间为2021/7/19 10:1:21 + 8小时
        rename = "时间触发器-新"
        self.scenario.update_trigger_time(
            "时间触发器", rename=rename, time="2021/7/19 11:1:21"
        )
        self.env.step()
        # 更新的时间为为实际时间，没有+8小时
    ...
    def test_add_trigger_unit_remains_in_area(self):
        ...
        stay_time = "0:1:2:3"
        self.scenario.add_trigger_unit_remains_in_area(
            "单元区域停留", target_filter_dict, area, stay_time
        )
        self.env.step()
    ...
    def test_update_trigger_unit_remains_in_area(self):
        ...
        stay_time = "1:2:3:4"
        rename = "单元区域停留-改"
        self.scenario.update_trigger_unit_remains_in_area(
            "单元区域停留",
            rename=rename,
            target_filter_dict=target_filter_dict,
            area=area,
    ...
    def test_add_trigger_unit_enters_area(self):
        ...
        self.scenario.add_trigger_unit_enters_area(
            "单元进入区域", target_filter_dict, area, ETOA, LTOA, trigger_if_not_in_area
        )
        self.env.step()
        # 实际设置时间为2021/7/19 10:1:21 + 8小时
    ...
    def test_update_trigger_unit_enters_area(self):
        ...
        self.scenario.update_trigger_unit_enters_area(
            "单元进入区域",
            rename=rename,
            target_filter_dict=target_filter_dict,
            area=area,
    ...
    def test_update_trigger_random_time(self):
        ...
        rename = "随机时间-改"
        self.scenario.update_trigger_random_time(
            "随机时间", rename=rename, earliest_time=earliest_time, latest_time=latest_time
        )
        self.env.step()
    ...
    def test_add_trigger_unit_detected(self):
        ...
        detector_side = "蓝方"
        # 不明
        self.scenario.add_trigger_unit_detected(
            "单元被探测到0", target_filter_dict, detector_side, 0
        )
        # 知道领域
        self.scenario.add_trigger_unit_detected(
            "单元被探测到1", target_filter_dict, detector_side, 1
        )
        # 知道类型
        self.scenario.add_trigger_unit_detected(
            "单元被探测到2", target_filter_dict, detector_side, 2
        )
        # 知道型号
        self.scenario.add_trigger_unit_detected(
            "单元被探测到3", target_filter_dict, detector_side, 3
        )
        # 知道具体ID
        self.scenario.add_trigger_unit_detected(
            "单元被探测到4", target_filter_dict, detector_side, 4
        )
        self.env.step()
    ...
    def test_update_trigger_unit_detected(self):
        ...
        detector_side = "蓝方"
        # 知道领域
        self.scenario.update_trigger_unit_detected(
            "单元被探测到0",
            rename="单元被探测到1",
            target_filter_dict=target_filter_dict,
    ...
    def test_update_action_points(self):
        ...
        rename = "蓝方得分-100"
        # 不明
        self.scenario.update_action_points(
            "红方得分100", rename=rename, side_name=detector_side, point_change=-100
        )
        self.env.step()
    ...
    def test_update_action_end_scenario(self):
        ...
        rename = "单元瞬时移动-改"
        self.scenario.update_action_teleport_in_area(
            "单元瞬时移动", rename=rename, unit_list=unit_list, area=area
        )
        self.env.step()
    ...
    def test_update_action_message(self):
        ...
        self.env.step()
        rename = "消息-改"
        self.scenario.update_action_message(
            "消息", rename=rename, side="蓝方", text="消息测试-改"
        )
        self.env.step()
    ...
    def test_update_action_change_mission_status(self):
        ...
        rename = "改变任务状态-改"
        self.scenario.update_action_change_mission_status(
            "改变任务状态", rename=rename, side="蓝方", mission="水上巡逻", new_status=1
        )
        self.env.step()
    ...
    def test_update_action_lua_script(self):
        ...
        rename = "lua脚本执行-改"
        self.scenario.update_action_lua_script(
            "lua脚本执行", rename=rename, script_text="ScenEdit_SetTrigger()"
        )
        self.env.step()
    ...
    def test_update_event_attribute(self):
        ...
        is_repeatable = "true"
        probability = 90
        self.scenario.update_event_attribute(
            "事件1",
    ...
    def test_update_condition_side_posture(self):
        ...
        rename = "推演方立场-改"
        self.scenario.update_condition_side_posture(
            "推演方立场",
            rename=rename,
            observer_side="蓝方",
            target_side="红方",
    ...
    def test_update_condition_scen_has_started(self):
        ...
        rename = "想定已加载-改"
        self.scenario.update_condition_scen_has_started(
            "想定已加载", rename=rename, is_reverse="false"
        )
        self.env.step()
    ...
    def test_update_condition_lua_script(self):
        ...
        rename = "lua脚本-改"
        self.scenario.update_condition_lua_script(
            "lua脚本", rename=rename, script_text=script_text
        )
        self.env.step()
    ...
    def test_set_scenario_time(self):
        ...
        self.scenario.set_scenario_time(
            scenario_current_time=scenario_current_time,
    ...
    def test_set_fineness(self):
        ...
        self.scenario.set_fineness(
            detailed_gun_fire_control=detailed_gun_fire_control,
