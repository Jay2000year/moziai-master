...
class MoziServer:
    ...
    def load_scenario(self):
        ...
        for i in range(60):
            ...
            if str(value) == "'Yes'":
               # by zhait
                with open('C:/Users/3-5/Desktop/moziai-master/mozi_ai_sdk/MoziDemo/output.txt', 'w+') as f:
                    f.write('%s：想定加载成功！'% (datetime.datetime.now()))
    ...
    def load_scenario_in_linux(self, path, model):
        ...
        参数：path {str: 想定文件的相对路径（仅支持XML文件）}
            bcmodel 模式 {str:  "Edit"-想定编辑模式 "Play"-想定推演模式}
        返回：lua返回结果信息 或 数据错误
        作者：-
        单位：北京华戍防务技术有限公司
        时间：4/2/20
        """
        cmd = "Hs_PythonLoadScenario('{}', '{}')".format(path, model)
